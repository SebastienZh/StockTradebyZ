...
class StockToolGUI:
    ...
    def start_fetch_data(self):
        ...
        args = {
            'datasource': self.datasource_var.get(),
            'frequency': frequency,
            'exclude_gem': self.exclude_gem_var.get(),
            'min_mktcap': min_cap,
            'max_mktcap': max_cap,
        ...
        # 如果是Tushare数据源，添加Token
        if datasource == "tushare":
            args['ts_token'] = self.ts_token_var.get().strip()
        
        # 启动数据抓取线程
        self.fetch_thread = threading.Thread(target=self.run_fetch_data, args=(args,))
    ...
    def run_fetch_data(self, args):
        """在后台线程中运行数据抓取"""
        try:
            self.log_message("开始抓取K线数据...")
            self.log_message(f"参数: {args}")
            
            # 导入fetch_kline模块
            import fetch_kline
            
            # 如果有Tushare Token，先设置它
            if 'ts_token' in args:
                fetch_kline.set_tushare_token(args['ts_token'])
                # 从args中移除token，避免传递给命令行参数
                del args['ts_token']
            
            # 临时修改sys.argv来模拟命令行参数
            old_argv = sys.argv.copy()
            sys.argv = ['fetch_kline.py']
            ...
            try:
                # 运行主函数
                fetch_kline.main()
                self.log_message("K线数据抓取完成!", "SUCCESS")
            finally:
                # 恢复原来的sys.argv
                sys.argv = old_argv
                
        except Exception as e:
            self.log_message(f"数据抓取失败: {str(e)}", "ERROR")
        finally:
            # 更新UI状态
    ...
    def start_stock_selection(self):
        """开始股票选择"""
        if hasattr(self, 'select_thread') and self.select_thread.is_alive():
        ...
        if self.select_date_var.get().strip():
            args['date'] = self.select_date_var.get().strip()
            
        # 启动选股线程
        self.select_thread = threading.Thread(target=self.run_stock_selection, args=(args,))
    ...
    def run_stock_selection(self, args):
        """在后台线程中运行股票选择"""
        try:
            self.log_message("开始股票选择...")
            self.log_message(f"参数: {args}")
            ...
            try:
                # 运行主函数
                select_stock.main()
                self.log_message("股票选择完成!", "SUCCESS")
            finally:
                # 恢复原来的sys.argv
                sys.argv = old_argv
                
        except Exception as e:
            self.log_message(f"股票选择失败: {str(e)}", "ERROR")
        finally:
            # 更新UI状态
            self.root.after(0, self.selection_completed)
